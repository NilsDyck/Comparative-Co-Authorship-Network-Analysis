...
files = os.scandir('data/raw/DLT')
sites = []
for entry in files:
    with open(os.path.join('data/raw/DLT', entry.name), 'r') as f:
       html = f.read()
       sites.append(html)

#initiate file to save data
with open('data/processed/DLT-DLT.csv', 'w') as f:
...
for site in sites:
    ...
    for paper in papers:
        ...
        f = open('data/processed/DLT-DLT.csv', 'a')
