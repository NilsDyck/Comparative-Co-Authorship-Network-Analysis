# Some modifications to the CIAA-CIAA dataset are done, to make work easier.
# Nils Dyck, 08.05.2023

import pandas as pd

df = pd.read_csv('data/cleaned/CIAA-CIAA.csv', sep=';')
key_table = pd.read_csv('data/processed/CIAA-authors.csv', sep=';')
...
with open('data/cleaned/CIAA-CIAA_m2.csv','w') as f: f.write('Author;Title;Co-Authors;No Authors;Year;Pagination\n')

for i in range(len(co_author_urls)):
    with open('data/cleaned/CIAA-CIAA_m2.csv','a') as f: f.write(str(author_urls[i])+';'+title[i]+';'+str(co_author_urls[i])+';'+str(no_authors[i])+';'+str(year[i])+';'+pagination[i]+'\n')
